		...
		<form action="/moim/create-task" method="post" class="moim-form"
			...
			<div class="block">
				<label>모임을 하고자하는 <span style="color: orangered;">날짜</span>와
					...
					<br/><small>(종료시간을 알수 없다면 설정을 비워두세요.)</small></label>
				<div class="block-row" style="flex: 1">
					<div class="block" style="flex: 2">
						<fmt:formatDate value="${today}" pattern="yyyy-MM-dd" var="defalutDate"/>
						<input type="date" style="text-align: center" name="date" value="${defalutDate}"/>
					</div>
					<div class="block" style="flex: 1">
						<input type="time" step="1800" style="text-align: center" name="begin" />
					</div>
					<div class="block" style="flex: 1">
						<input type="time" step="1800" style="text-align: center" name="end" />
					</div>
			...
			<p style="color: red;">모임개설에 실패하였습니다.</p>
			</c:if>
			<div class="block">
